...
class NimWSSServer:
    ...
    def _build_nim_system_prompt(self, client: ClientInfo) -> str:
        ...
        return (
            ...
            "path absoluto, content solo para write).\n"
            "• nim_patch_file: editar archivos — parámetros path, target "
            "(texto exacto a buscar), replacement (texto nuevo). NO uses "
            "old_string/new_string — la app acepta target/replacement.\n"
            "• nim_grep_search: buscar texto/regex en archivos — parámetros "
            "query y path (directorio raíz). NO uses pattern.\n"
            "• nim_file_ops: operaciones de archivo — action (move|copy|mkdir|"
            "delete|exists|size), path, dest (solo move/copy).\n"
            "• nim_code_exec: ejecutar código en la PC — lang (python|node), "
            "code, timeout_secs (máx 30).\n"
            "• nim_checkpoint: snapshots — action (create|list|restore), path, "
            "label.\n"
            "• nim_computer_use: control de pantalla — action (screenshot|"
            "click|move|type), x/y, text.\n"
            "• nim_antigravity: ejecutar Antigravity CLI — prompt, cwd, "
            "timeout_secs.\n"
            "• NO disponibles en la app actual (responden 'not implemented'): "
            "nim_browser, nim_git_status, nim_git_diff, nim_git_commit — NO "
            "las uses.\n"
