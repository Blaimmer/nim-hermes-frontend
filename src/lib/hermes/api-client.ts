...
// ── Adaptación NIM PC (Tauri): HermesApiRequest local ─────────────────────
// La app oficial usa window.hermesDesktop.api (bridge Electron). NIM PC es
// Tauri/React: hermesApi() hace fetch directo contra el backend hermes serve.
...
/** Send a REST request to the backend (hermes serve).
 *
 *  Adaptación NIM PC: la app oficial delega en el bridge Electron
 *  (window.hermesDesktop.api). NIM PC es Tauri/React → fetch directo.
 *  La base URL se configura con NIM_SERVE_URL (default http://127.0.0.1:9119).
 *  En Tauri, el frontend puede leerla vía env del backend o variable global.
 */
let _serveBaseUrl = 'http://127.0.0.1:9119'

export function setServeBaseUrl(url: string): void {
  _serveBaseUrl = url.replace(/\/+$/, '')
}
...
/** Cookie de sesión del backend (basic auth → cookie). Se guarda tras login()
 *  y se envía en cada hermesApi(). En Tauri usamos el store de cookies local;
 *  en navegador/web, document.cookie. */
let _sessionCookie = ''
...
export async function login(username: string, password: string): Promise<void> {
  const res = await fetch(`${_serveBaseUrl}/auth/password-login`, {
    method: 'POST',
...
export function isAuthenticated(): boolean {
  return _sessionCookie.length > 0
}

export async function hermesApi<T>(request: HermesApiRequest): Promise<T> {
  const { path, method = 'GET', body, timeoutMs } = request
  const url = `${_serveBaseUrl}${path.startsWith('/') ? path : `/${path}`}`
  ...
  try {
    ...
    if (!res.ok) {
      const text = await res.text().catch(() => '')
      throw new Error(`Hermes API ${res.status}: ${text.slice(0, 200)}`)
    }
...
function isScopeObject(scope: ProfileScope | undefined): scope is { connectionId?: null | string; profile?: null | string } {
...
export function capabilityScoped(scope?: ProfileScope): { connectionId?: string; profile?: string } {
  ...
  return { ...profileScoped(scope as null | string | undefined), ...connectionScoped() }
}
...
export function profileScopeKey(scope?: ProfileScope): string {
  ...
  return ((scope as null | string | undefined) ?? '').trim() || 'default'
}
